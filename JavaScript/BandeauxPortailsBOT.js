...
(function () {      
    ...
    function modifierPortail(texte, portailNom) {      
        ...
        while ((match = regex.exec(texteNet)) !== null) {      
            let currentParams = [];      
            if (match[1]) currentParams = match[1].split('|').map(p => p.trim()).filter(Boolean);      
            else if (match[2]) currentParams = [match[2].trim()];      
            const paramsNorm = normaliser(currentParams);      
            if (paramsNorm.has(portailLower)) return { text: texte, modified: false };      
            paramsList = currentParams.concat(portailNom.trim());      
            texte = texte.replace(match[0], '{{Portail|' + paramsList.join('|') + '}}');      
            found = true;      
        }      
