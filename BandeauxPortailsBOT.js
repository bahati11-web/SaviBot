// BandeauxPortailsBOT.js | Version 1.1 | Bahati11
...
(function () {
    ...
    function modifierPortail(texte, portailNom) {
        ...
        const regex = /\{\{\s*portail\s*(\|[\s\S]*?)?\}\}/i;
        const match = regex.exec(texteNet);

        if (match) {
            const modele = match[0];
            ...
            if (paramsNorm.has(portailLower)) {
                return { text: texte, modified: false };
            }

            paramsList.push(portailNom.trim());
            const nouveau = '{{portail|' + paramsList.join('|') + '}}';
            return { text: texte.replace(modele, nouveau), modified: true };
        }
